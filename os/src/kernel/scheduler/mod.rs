...
use crate::{kernel::{scheduler::rr_scheduler::RRScheduler, TaskStruct}, sync::spin_lock::SpinLock};
...
pub trait Scheduler {
    ...
    // 任务管理
    fn add_task(&mut self, task: Arc<TaskStruct>); // 任务首次创建或从阻塞队列返回
    fn next_task(&mut self) -> Arc<TaskStruct>; // 选择下一个要运行的任务
