...
use crate::{device::net::{network_config::NetworkConfigManager, network_interface::NETWORK_INTERFACE_MANAGER}, println};
...
/// 设置网络接口配置
pub fn set_network_interface_config(ifname: *const c_char, ip: *const c_char, gateway: *const c_char, mask: *const c_char) -> isize {
    // 解析参数
    unsafe {
        sstatus::set_sum();
        ...
        // 设置网络配置
        match NetworkConfigManager::set_interface_config(
            ifname_str,
            ip_str,
            gateway_str,
            mask_str
...
// 获取网络接口配置
pub fn getsockopt(sockfd: i32, level: i32, optname: i32, optval: *mut u8, optlen: *mut u32) -> isize {
    unsafe {
        sstatus::set_sum();
...
// 发送数据到指定地址
pub fn sendto(sockfd: i32, buf: *const u8, len: usize, flags: i32, dest_addr: *const u8, addrlen: u32) -> isize {
    unsafe {
        sstatus::set_sum();
...
pub fn recvfrom(sockfd: i32, buf: *mut u8, len: usize, flags: i32, src_addr: *mut u8, addrlen: *mut u32) -> isize {
    unsafe {
        sstatus::set_sum();
