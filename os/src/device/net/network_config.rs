...
use smoltcp::wire::{IpCidr, Ipv4Address, IpAddress};
...
impl NetworkConfigManager {
    ...
    pub fn set_ip_address(interface_name: &str, ip: &str, prefix: u8) -> Result<(), NetworkConfigError> {
        // 查找指定接口
        if let Some(interface) = NETWORK_INTERFACE_MANAGER.lock().find_interface_by_name(interface_name) {
            // 解析IP地址
    ...
    pub fn get_interface_config(interface_name: &str) -> Result<String, NetworkConfigError> {
        if let Some(interface) = NETWORK_INTERFACE_MANAGER.lock().find_interface_by_name(interface_name) {
            ...
            config.push_str(&alloc::format!("MAC Address: {}\n", interface.mac_address()));
    ...
    pub fn set_interface_config(
        ...
        if let Some(interface) = NETWORK_INTERFACE_MANAGER.lock().find_interface_by_name(interface_name) {
            ...
            println!("Set interface config for {}: IP={}/{}, Gateway={}", interface_name, ip, prefix_length, gateway);
            Ok(())
        } else {
            Err(NetworkConfigError::InterfaceNotFound)
        }
