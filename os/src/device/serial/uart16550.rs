...
use crate::{device::{DRIVERS, DeviceType, Driver, SERIAL_DRIVERS, device_tree::DEVICE_TREE_REGISTRY, serial::SerialDriver}, earlyprintln, kernel::current_memory_space, mm::address::{Paddr, UsizeConvert}, sync::SpinLock};
...
pub fn init(node: &FdtNode) {
    let reg = node.reg().and_then(|mut reg| reg.next()).expect("No reg property found for ns16550a");
    let paddr = reg.starting_address as usize;
    let size = reg.size.unwrap_or(0);
    if size == 0 {
        earlyprintln!("[Device] ns16550a device tree node {} has no size", node.name);
        return;
    }
    ...
    serial_port.init();
    let driver = Arc::new(Uart16550 { serial_port: SpinLock::new(serial_port) });
