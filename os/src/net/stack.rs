...
use crate::net::{interface::NetworkInterface, protocol::{arp, icmp, EthernetHeader}};
...
impl NetworkStack {
    ...
    /// 处理ARP数据包
    fn process_arp_packet(&mut self, interface: &Arc<Mutex<NetworkInterface>>, eth_header: &EthernetHeader, payload: &[u8]) {
        if payload.len() < 28 {
            return;
        }
        ...
        let arp_packet = arp::ArpPacket {
            ...
            sender_mac: [payload[8], payload[9], payload[10], payload[11], payload[12], payload[13]],
            sender_ip: [payload[14], payload[15], payload[16], payload[17]],
            target_mac: [payload[18], payload[19], payload[20], payload[21], payload[22], payload[23]],
            target_ip: [payload[24], payload[25], payload[26], payload[27]],
        };
        
        // 更新ARP缓存
        self.arp_cache.add_entry(arp_packet.sender_ip, arp_packet.sender_mac);
    ...
    /// 处理IPv4数据包
    fn process_ipv4_packet(&mut self, interface: &Arc<Mutex<NetworkInterface>>, eth_header: &EthernetHeader, payload: &[u8]) {
        if payload.len() < 20 {
            return;
        }
    ...
    /// 处理ICMP数据包
    fn process_icmp_packet(&mut self, interface: &Arc<Mutex<NetworkInterface>>, eth_header: &EthernetHeader, payload: &[u8]) {
        if payload.len() < 8 {
            return;
        }
    ...
    /// 发送ARP请求
    pub fn send_arp_request(&mut self, interface: &Arc<Mutex<NetworkInterface>>, target_ip: [u8; 4]) {
        let interface_lock = interface.lock();
    ...
    /// 发送ICMP回显请求（ping）
    pub fn send_icmp_echo_request(&mut self, interface: &Arc<Mutex<NetworkInterface>>, target_ip: [u8; 4], identifier: u16, sequence: u16) {
        // 检查ARP缓存
        if let Some(target_mac) = self.arp_cache.find_mac(target_ip) {
            let interface_lock = interface.lock();
            
            // 构建ICMP请求
            let icmp_request = icmp::IcmpPacket::new_echo_request(identifier, sequence, b"Hello, World!");
