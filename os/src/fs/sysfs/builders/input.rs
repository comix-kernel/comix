//! 输入设备 sysfs 树构建器
//!
//! 在 /sys/class/input/ 下创建指向 /sys/devices/platform/ 的符号链接
...
/// 构建输入设备 sysfs 树
///
/// 在 /sys/class/input/ 下创建指向 /sys/devices/platform/<device>/ 的符号链接
pub fn build_input_devices(root: &Arc<SysfsInode>) -> Result<(), FsError> {
    ...
    for dev_info in device_registry::list_input_devices() {
        // 创建符号链接: /sys/class/input/input0 -> ../../devices/platform/input0
