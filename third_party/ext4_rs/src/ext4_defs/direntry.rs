...
impl Ext4DirEntry {
    ...
    pub fn ext4_dir_get_csum(&self, s: &Ext4Superblock, blk_data: &[u8], ino_gen: u32) -> u32 {
        ...
        let mut data = [0u8; BLOCK_SIZE - core::mem::size_of::<Ext4DirEntryTail>()];
        let len = core::cmp::min(data.len(), blk_data.len());
        data[..len].copy_from_slice(&blk_data[..len]);

        csum = ext4_crc32c(csum, &data[..], data.len() as u32);
        csum
    }
